 ...
 * <p>
 * Endpoints: - POST /api/affiliate/tracking/create-link - Create tracking link - POST
 * /api/affiliate/tracking/estimate-cashback - Estimate cashback amount - GET
 * /api/affiliate/tracking/redirect/{clickId} - Handle click redirect
 * <p>
...
public class AffiliateTrackingController {
   ...
   * <p>
   * SECURITY UPDATE: - This endpoint now REQUIRES authentication (JWT token) - User ID is extracted
   * from JWT token, NOT from request body - This prevents users from creating tracking links for
   * other users
   * <p>
   * User provides Shopee product URL, system generates affiliate tracking link.
   * <p>
   * Flow: 1. User logs in → receives JWT token from Keycloak 2. User copies Shopee product URL 3.
   * User pastes URL into CashBee app 4. Frontend sends request with JWT token in Authorization
   * header 5. Backend extracts user ID from JWT token (secure, cannot be forged) 6. System
   * generates tracking link for authenticated user 7. User clicks tracking link → redirects to
   * Shopee with tracking
   *
   ...
   * <p>
   * Formula: - commissionRate = sellerCommissionRate + shopeeCommissionRate - estimatedCashback =
   ...
   * <p>
   * When user clicks tracking link, this endpoint: 1. Records the click event (status → CLICKED) 2.
   * Redirects user to actual Shopee affiliate URL
   * <p>
   ...
   * <p>
   * Allows redirect by tracking code instead of click ID. Useful if frontend stores tracking code
   * instead of click ID.
   *
   ...
   * <p>
   * Returns the tracking URL without performing redirect. Useful for testing or showing link
   * preview to user.
   *
