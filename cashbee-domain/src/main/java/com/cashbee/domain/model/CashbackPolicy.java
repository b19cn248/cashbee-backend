...
/**
 * CashbackPolicy Domain Model.
 * Pure business object without any infrastructure concerns.
 *
 * Represents a cashback policy that defines how much cashback users receive
 * from affiliate commissions. Different policies can apply based on:
 * - Platform (Shopee, Lazada, etc.)
 * - User level (NORMAL, VIP, SUPER)
...
public class CashbackPolicy {
    ...
    /**
     * Policy code for identification (e.g., "VIP_SHOPEE_2025").
     * Must be unique.
     */
     ...
     * Foreign key to affiliate_platform table.
     */
    private Long platformId;

    /**
     * User level this policy applies to.
     * Different levels get different cashback rates.
     */
    private UserLevel userLevel;

    /**
     * Cashback rate as percentage of commission.
     * Example: 70.00 means user gets 70% of commission as cashback.
     * Range: 0.00 - 100.00
     */
     ...
     * Null means no cap.
     * Example: If set to 100,000 VND, user can't get more than that per order.
     */
     ...
     * Policy only applies from this date onwards.
     */
    ...
    /**
     * Check if policy is effective right now.
     * Policy must be:
     * - Active
     * - Started (effectiveFrom <= now)
     * - Not expired (effectiveTo >= now or null)
     *
    ...
    public boolean isEffectiveNow() {
        ...
        // Must not be expired
        if (this.effectiveTo != null && this.effectiveTo.isBefore(now)) {
            return false;
        }
    ...
    /**
     * Calculate cashback amount from commission amount.
     * Applies cashback rate and respects max cashback cap.
     *
     * Formula: cashback = commission * (rate / 100)
