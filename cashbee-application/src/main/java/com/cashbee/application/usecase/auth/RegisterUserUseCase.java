 ...
 * <p>
 * NEW FLOW (with OTP verification): 1. Validates input (username, email, password, referral code)
 * 2. Checks for duplicates in Keycloak (username, email) 3. Creates user in Keycloak with
 * ENABLED=FALSE (user cannot login yet) 4. Generates 6-digit OTP code 5. Saves OTP record with
 * keycloakId and registration data (JSON without password) 6. Sends OTP email to user 7. Returns
 * response indicating OTP verification is required
 * <p>
 * User must then call VerifyOtpUseCase to: - Enable Keycloak user - Create local DB user - Create
 * wallet - Complete registration
 * <p>
 * Transaction Management: - Uses @Transactional for OTP save - If OTP save fails, cleanup Keycloak
 * user
 * <p>
 * Error Handling: - Duplicate username/email → BusinessException - Invalid referral code →
 * BusinessException - Keycloak creation failure → RuntimeException - OTP save failure →
 * RuntimeException (with cleanup) - Email send failure → RuntimeException (with cleanup)
 *
 * @author CashBee Team
 */
@Service
...
public class RegisterUserUseCase {
  ...
  public RegisterResponse execute(RegisterRequest request) {
    ...
    try {
      ...
      throw new BadRequestException("Failed to initiate registration", e);
    }
    ...
    try {
      ...
      throw new BadRequestException("Failed to send verification email. Please try again.", e);
    }
    ...
    log.info("UseCase: Registration initiated, OTP verification required for: {}",
        request.getEmail());
    return response;
  }
  ...
  private void validateReferralCode(RegisterRequest request) {
    ...
    if (!referralCodeGenerator.isValidFormat(referralCode)) {
      ...
      throw new BusinessException(
          "Invalid referral code format. Must be CB followed by 6 alphanumeric characters.");
    }
  ...
  /**
   * Create DISABLED user in Keycloak with password. User will be enabled after OTP verification.
   * <p>
  ...
  private String createDisabledKeycloakUser(RegisterRequest request) {
    ...
    try {
      ...
      throw new BadRequestException("Failed to create user in authentication system", e);
    }
  }

  /**
   * Save OTP verification record with registration data. Registration data is stored as JSON
   * (without password).
   *
   * @param request    Registration request
